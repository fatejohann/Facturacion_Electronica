let lastToken = 'Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIwNjE0MjgwMzkwMTEyMSIsImF1dGhvcml0aWVzIjpbIlVTRVIiLCJVU0VSX0FQSSIsIlVzdWFyaW8iXSwiaWF0IjoxNzE2NjkwNDAzLCJleHAiOjE3MTY3NzY4MDN9.BLKVriFHreBNvwYdRsevi-A79Py3XloV-89K4I6cIthugQtw53gzt6bk1yV9G7Sku1bmGjCDNgTEEAwMeJj_dg'; // Variable global para almacenar el último token
            ...
            facturaJson = {
                ...
                "dteJson": {
                    "identificacion": {
                        ...
                        "ambiente": "00",
                        "tipoDte": "03",
                        "numeroControl": "DTE-01-0001ONEC-000000000005037",
...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("submitDocumentButton").addEventListener("click", async function() {
        try {
            ...
            const tipodte=facturaJson.dteJson.identificacion.tipoDte;

            // Generar el nuevo número de control
            facturaJson.dteJson.identificacion.numeroControl = generarNuevoNumeroControl(facturaJson.dteJson.identificacion.numeroControl, tipodte);
            ...
            facturaJson.dteJson.identificacion.fecEmi = new Date().toISOString().split('T')[0]; // Actualiza la fecha de emisión
            facturaJson.dteJson.identificacion.horEmi = new Date().toLocaleTimeString('en-GB'); // Actualiza la hora de emisión

            // Envía la factura
            await enviarFactura(facturaJson);

            // Si la factura se envió correctamente, puedes sobreescribir el número de control en el JSON original
            facturaJson.dteJson.identificacion.numeroControl = generarNuevoNumeroControl(facturaJson.dteJson.identificacion.numeroControl, documentoSeleccionado);
...
async function enviarFactura(facturaJson, documentoSeleccionado) {
    try {
        ...
        const dataToSend = [fechaCreacion, estado, selloRecibido, codigoGeneracion, fechaEmitido, documentoSeleccionado];
...
function generarNuevoNumeroControl(numeroControlActual, tipodte) {
    ...
    const nuevoNumeroControl = `DTE-${pad(tipodte, 2)}-0001ONEC-${pad(numero.toString(), 15, '0')}`;
